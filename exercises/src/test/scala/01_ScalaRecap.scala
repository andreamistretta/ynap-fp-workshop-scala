...
object ScalaRecap extends SimpleTestSuite {
  ...
  trait Fruit {
    ...
    def eatenBy(input: String): String = s"$input ate $stringify"
  }
  ...
  case class Person(name: String, age: Int) {
    ...
    def copy(age: Int) = Person(name, age)

    def eat[A <: Fruit](input: A): String =
      input.eatenBy(name)
  ...
  implicit class PersonExt(p: Person) {
    def toMap: Map[String, String] = Map("name" -> p.name, "age" -> p.age.toString)
  }

  object Person {
    def create(nameAndAge: String): Person = {
      val array: Array[String] = nameAndAge.split(", ")
      Person(array(0), array(1).toInt)
    }
